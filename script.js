...
function renderProjectFilters(rows){const cats=['all',...new Set(rows.map(x=>x.category||'Other'))];const el=$('#projectFilters');if(!el)return;el.innerHTML=cats.map(c=>'<button class="filter-chip '+(c==='all'?'active':'')+'" data-filter="'+escapeHtml(c)+'">'+escapeHtml(c==='all'?'All work':c)+'</button>').join('');$('.filter-chip').forEach(b=>b.addEventListener('click',()=>{ $('.filter-chip').forEach(x=>x.classList.remove('active'));b.classList.add('active');const f=b.dataset.filter;$('.project').forEach(p=>p.style.display=(f==='all'||p.dataset.category===f)?'':'none');beep(480,.04)}))}
